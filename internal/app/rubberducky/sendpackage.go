...
func SendPackage(packagename string, password string, t *big.Int, ec *ethclient.Client) error {
	...
	wa := ks.Accounts()
	var non uint64
	non, err = ec.NonceAt(context.Background(), wa[4].Address, nil)
	if err != nil {
		err = fmt.Errorf("Internal error: '%v'", err)
		return err
	}
	...
	stx, err = ks.SignTxWithPassphrase(wa[4], password, tx, t)
	if err != nil {
